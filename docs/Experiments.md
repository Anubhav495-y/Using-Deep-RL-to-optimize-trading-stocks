...
## 11. Traditional Forecasting Benchmarks (ARIMA & LSTM)

Under the same walk-forward validation protocol (identical transaction costs, slippage, and windows), we evaluated **ARIMA(1, 0, 1)** on daily returns and a **PyTorch LSTM Regressor** (sequence length 10, State 2 features) across all 4 tickers.

Detailed mathematical descriptions and training specifications are registered in [Benchmarks ARIMA LSTM](file:///mnt/c/Users/Saket/Desktop/Projects/Finsearch_RL/docs/Benchmarks_ARIMA_LSTM.md).

---

## 12. Research Insights from Traditional Benchmarks

1. **ARIMA acts as a low-risk momentum proxy**:
   - ARIMA generated positive returns (28% to 35%) across all four assets, outperforming Buy & Hold on HDFC Bank (+35.38% vs. +29.59%).
   - **Why it worked**: Modeling short-term autocorrelation allowed the ARIMA model to shift holdings quickly and avoid downswings, though its lack of risk-minimizing constraints led to high drawdowns similar to passive benchmarks.

2. **LSTM return regression collapses under noise**:
   - The deep LSTM model failed to beat any benchmark, ending near 0% return on TCS, HDFC Bank, and Infosys.
   - **Why it failed**: Traditional regression models trained on Mean Squared Error (MSE) target daily stock returns, which are dominated by high-frequency noise. The model overfit this noise to minimize MSE. Lacking a risk-aware objective (like Sharpe/Sortino penalties) or trading decision boundaries, it traded hyperactively and lost all profits to transaction costs and slippage.

3. **Superiority of RL Policy Learning**:
   - The RL agents outperformed ARIMA and LSTM on all assets by a wide margin. This demonstrates that learning a direct trading policy under transaction fee friction and risk constraints is far superior to standard forecasting.
