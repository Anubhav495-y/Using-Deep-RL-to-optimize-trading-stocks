...
* **State 7 (Market Context - Multi-Asset Correlation)**: Incorporates NIFTY 50 index context:
  ...
  Adding market context regularized the policy on HDFCBANK and INFY, letting the agent filter out stock-specific volatility.
...
Optimizing directly for downside risk variance prevented the agent from holding failing positions, lowering max drawdown and significantly boosting out-of-sample Sortino and Sharpe ratios.
...
### 1. Discrete DQN (Tuned)
* **Action Space**: `discrete_3` (Hold, 100% Buy, 100% Sell).
* **Why it works**: By restricting the action space to ternary bounds, we regularize the policy, preventing the agent from over-trading. Optuna-tuned DQN parameters (`batch_size: 256`, `gamma: 0.965`, `buffer_size: 5000`) break temporal correlation in financial time series via the replay buffer, making DQN the discrete champion on TCS, RELIANCE, and HDFCBANK.

### 2. Continuous SAC & TD3
* **Action Space**: `continuous` (Box $[-1, 1]$ representing trade fractions).
* **Stochastic SAC**: Outputs action distributions. Without risk regularization, SAC trades hyperactively (852 trades on HDFCBANK), losing profit to transaction fees. However, under `portfolio_return` on INFY, SAC captured massive alpha (**108.27% return**).
* **Deterministic TD3**: Employs deterministic policy smoothing and delayed policy updates. By smoothing actions, TD3 regularized position scaling, achieving **51.42% return** in only **19 trades** on INFY (drawdown: **-10.68%**).
...
| | **Discrete DQN (Tuned)** | **State 2** | **47.12%** | **0.31** | **-16.64%** | **14** |
...
1. **Outperforming Forecasting (ARIMA/LSTM)**: ARIMA and LSTM models attempt to predict raw price returns, resulting in hyperactive whipsawing (e.g. 91 trades for LSTM on TCS) that gets eroded by transaction costs. Our RL framework trains the policy directly on downstream portfolio returns under fee constraints, teaching the model to trade selectively.
2. **Sortino Stabilization**: On HDFC Bank, discrete DQN under standard rewards failed (-0.95% return). Switching to the **Differential Sortino** reward under State 7 context regularized the policy, boosting return to **51.73%** and keeping drawdown lower than index buy-and-hold (-20.87% vs. -23.24%).
